...
function register(api: PluginApi) {
  ...
  const collection = (cfg.collectionName as string) || process.env.QDRANT_COLLECTION || "family_memory";
      ...
      const searchTool: AgentTool = {
        ...
        execute: async (_tid, params) => {
          try {
            ...
            if (!text) text = "No matching memories found in this household.";
      ...
      const revokeTool: AgentTool = {
        ...
        execute: async (_tid, params) => {
          try {
            ...
            if (count === 0) return ok("No matching memories found that you own.");
      ...
      const alertsTool: AgentTool = {
        ...
        execute: async (_tid, params) => {
          try {
            ...
            if (alerts.length === 0) return ok("No pending alerts.");
            ...
            if (params.mark_read) await memory.markAlertsRead(caller);
      ...
      const statsTool: AgentTool = {
        ...
        execute: async () => {
          try {
            ...
            return ok(
              `Household memory stats:\n- Tenant: ${tenantId}\n- Total memories: ${stats.totalPoints}\n- Pending alerts: ${stats.totalAlerts}\n- Status: ${stats.status}\n- Your identity: ${caller}\n- Qdrant multitenancy: is_tenant=true on tenant_id, per-tenant HNSW (m=0, payload_m=16)`,
              stats,
            );
          } catch (err) {
